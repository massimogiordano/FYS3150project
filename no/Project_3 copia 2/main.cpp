...
/*
 * The next function generate a matrix call y_i with all velocity
 * and position. A temporaneal matrix. A matrix call k1, k2,k3 and k4
 *
 */
    mysystem.generate_matrixs(mysystem.all_planets);
...
cout << "succede qualcosa??"<< endl;
   ...
   while(t<tmax){
       ...
       for(int i=0; i<7; i++){
            for(int j=0; j<elements; j++){
             cout << mysystem.y_i[j][i]<< " y_i ";
            }
        cout << endl;
       }


        cout << mysystem.k1[6][0] <<"prima"<< endl;
        derivate(mysystem.y_i, mysystem.k1, elements);
        cout << mysystem.k1[0][6] << "dopo"<< endl;
        cout << "massa " << sun.mass<< endl;
